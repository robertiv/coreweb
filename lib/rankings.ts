...
async function queryUniquesRanking() {
	const pool = await getPool();
	const result = await pool.request().query(
		"SELECT ROW_NUMBER() OVER (ORDER BY r.Points DESC) AS RankNo, c.CharName16 as charname, CASE WHEN RefObjID < 5000 THEN 'chinese' ELSE 'european' END + ' ' + CASE WHEN Strength > Intellect THEN 'str' else 'int' end as class, t.jobType as jobType, r.Points as uniquepoints, case when g.[Name] = 'dummy' then '<none>' else g.[Name] end as guildname, c.LastLogout FROM SRO_VT_SYSTEMS.dbo._UniquesRank r JOIN SRO_VT_SHARD.dbo._Char c ON c.CharID = r.CharID JOIN SRO_VT_SHARD.dbo._Guild g ON c.GuildID = g.ID JOIN SRO_VT_SHARD.dbo._CharTrijob t ON t.CharID = c.CharID ORDER BY r.Points DESC",
	);
